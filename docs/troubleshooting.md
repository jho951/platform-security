...
- `authentication.required`가 `true`인지 확인한다.
...
- `ip-guard.allowed-ips` 값이 실제 client IP와 같은 포맷인지 확인한다.
- trusted proxy 뒤에 있다면 `client IP 해석` 단계가 먼저 적용되는지 확인한다.

## 3. rate limit이 너무 빨리 막힌다

- `rate-limit.limit`과 `rate-limit.window`를 확인한다.
...
- `platform-security-autoconfigure`와 `platform-security-starter`가 둘 다 classpath에 있는지 확인한다.
- published artifact 버전이 서로 맞는지 확인한다.
