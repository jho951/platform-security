...
`platform-security-adapter-auth` 공개 계약은 raw `com.auth.*` 대신 platform-owned runtime view와 port를 기준으로 잡는다. 이 view는 orchestration 목적이어야 하며, 1계층 principal/service 계약을 이름만 바꿔 다시 만드는 canonical model이 되면 안 된다. raw auth SPI는 adapter/helper 계층에서만 남긴다.
...
`TokenService`, `SessionStore`, `RateLimiter` 같은 1계층 타입은 필요하면 adapter layer 내부에서만 연결한다. service-facing bean, policy, 운영 안전검사는 raw SPI가 아니라 platform port 계약만 본다.
...
`platform-security-support-local`는 local/test 전용 기본 구현이다. 운영 `implementation`에 넣지 않는다.

`platform-security-hybrid-web-adapter`는 gateway가 hybrid mode에서 `SecurityIngressAdapter`와 web ingress filter를 함께 직접 조립할 때 쓰는 공식 통합 표면이다. 이 모듈은 Servlet용 `PlatformSecurityGatewayIntegration`과 WebFlux용 `PlatformSecurityReactiveGatewayIntegration` bean을 제공한다.
