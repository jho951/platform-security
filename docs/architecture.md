...
`platform-security-auth` 공개 계약은 raw `com.auth.*` 대신 `PlatformAuthenticatedPrincipal`, `PlatformOAuth2UserIdentity` 같은 platform-owned 타입을 기준으로 잡는다. raw auth SPI는 adapter/helper 계층에서만 남긴다.
...
`RateLimiter`는 필요하면 `PlatformRateLimitAdapter` 내부 구현에 연결하는 helper input으로만 쓴다. policy와 운영 안전검사는 raw limiter가 아니라 platform adapter 계약만 본다.
...
`platform-security-hybrid-web-adapter`는 gateway가 hybrid mode에서 `SecurityIngressAdapter`, servlet filter, gateway header filter를 함께 직접 조립할 때 쓰는 공식 통합 표면이다. 이 모듈은 `PlatformSecurityGatewayIntegration` bean을 제공한다.
